...
if [ -z ${MDP_SERVER_DOMAIN} ]; then
  echo "MDP_SERVER_DOMAIN environment variable is not set!"
  exit 1
fi
...
check_image_version "https://${MDP_SERVER_DOMAIN}/health" "${IMAGE_TAG}" 60 5 || SHOULD_EXIT_WITH_CODE=1
...
# curl https://mdp.example.com/health
# {
#   "status": "OK",
