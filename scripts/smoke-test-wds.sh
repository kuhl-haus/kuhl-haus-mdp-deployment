...
if [ -z WDS_SERVER_DOMAIN ]; then
  echo "WDS_SERVER_DOMAIN environment variable is not set!"
  exit 1
fi
# Stack User Home Directory
export BASE_WORKING_DIR="${GO_BASE_DIR}/pipelines/${GO_PIPELINE_NAME}"
export DEPLOY_DIR_NAME="deploy"
...
check_image_version "https://${WDS_SERVER_DOMAIN}/health" "${IMAGE_TAG}" 60 5 || SHOULD_EXIT_WITH_CODE=1
...
# curl https://wds.example.com/health
# {
#     "status":"OK",
